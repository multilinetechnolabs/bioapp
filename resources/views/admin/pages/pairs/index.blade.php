...
@section('content')
    ...
    <div id="content-container">
        <div class="admin-page-header">
            <h2 class="admin-page-title">{{ __('Pairs') }}</h2>
            <div class="admin-page-header__actions">
                <button type="button" class="admin-btn admin-btn--outline" onclick="window.location.href='/admin/pairs/import'">
                ...
                <button type="button" class="admin-btn admin-btn--primary" data-toggle="modal" data-target="#pairModal" data-title="New Pair">
        ...
        $(document).ready(function() {
            $('#pairModal').on('show.bs.modal', function(e) {
                ...
                if (id) {
                    ...
                    $('#pair_id, #ref_no, #name, #radical, #origins, #symptoms, #paths, #alternative_routes').val('');
            ...
            $('.save-btn').click(function(e) {
                ...
                var data = { scan_type: $('#scan_type').val(), ref_no: $('#ref_no').val(), name: $('#name').val(), radical: $('#radical').val(), origins: $('#origins').val(), symptoms: $('#symptoms').val(), paths: $('#paths').val(), alternative_routes: $('#alternative_routes').val() };
            ...
            $('#pairs').DataTable({
                ...
                columns: [
                    ...
                    { data: 'scan_type', render: function(d) { return d === 'body_scan' ? 'Body Scan' : d === 'chakra_scan' ? 'Chakra Scan' : d; } },
                    { data: 'name', name: 'name' },
                    ...
                    { data: 'alternative_routes', name: 'alternative_routes' },
                    {
                        ...
                        render: function(data) {
                            return '<div class="admin-action-group">'
                                ...
                                + '<button class="admin-action-btn admin-action-btn--edit editor-edit" data-toggle="modal" data-target="#pairModal" data-title="Edit Pair" data-id="' + data + '">' + icEdit + '</button>'
                                + '</div>';
                        }
                    }
                ]
