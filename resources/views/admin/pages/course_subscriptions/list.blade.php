...
@section('page-title')Course Subscriptions@stop
@section('styles')
    @parent
@stop
@section('content')
    ...
    <div id="content-container">
        <div class="admin-page-header">
            <h2 class="admin-page-title">{{ __('Course Subscriptions') }}</h2>
        </div>
        <div class="admin-dt-wrap table-responsive">
            <table class="table table-hover table-bordered" id="course_subscriptions">
                <thead>
                    <tr>
                        <th>{{ __('ID') }}</th>
                        <th>{{ __('User') }}</th>
    ...
    <div class="modal fade" id="courseSubscriptionModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="courseSubscriptionModalTitle"></h5>
                    <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
                </div>
                <form>
                    <div class="modal-body">
                        <input type="hidden" id="course_subscription_id" name="course_subscription_id">
                        <input type="hidden" id="user_id" name="user_id">
                        <div class="form-group">
                            <label>{{ __('Course') }}</label>
                            <select class="form-control" id="course_id" name="course_id" required>
                                <option value=""></option>
                                @foreach(\App\Models\Course::orderBy('title', 'asc')->get() as $course)
                                    <option value="{{ $course->id }}">{{ ucfirst($course->title) . ' ($' . number_format($course->price, 2) . ')' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
...
@section('javascripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#courseSubscriptionModal').on('show.bs.modal', function(e) {
                var trigger = $(e.relatedTarget);
                $('#courseSubscriptionModalTitle').text(trigger.data('title'));
                var id = trigger.data('id');
                if (id) {
                    $.ajax({
                        url: '{{ env("APP_WEB_API_URL") }}/{{ env("APP_WEB_API_VERSION") }}/course-subscriptions/' + id,
                        type: 'GET',
                        success: function(r) {
                            $('#course_subscription_id').val(id);
                            $('#user_id').val(r.user_id);
                            $('#starts_at').val(formatDate(r.starts_at));
                            $('#ends_at').val(formatDate(r.ends_at));
                            $('#course_id').val(r.course_id);
                        }
                    });
                } else {
                    $('#course_subscription_id, #user_id, #starts_at, #ends_at').val('');
                    $('#course_id').val('');
                }
            });
            ...
            $('.save-btn').click(function(e) {
                e.preventDefault();
                var id = $('#course_subscription_id').val();
                var data = { user_id: $('#user_id').val(), course_id: $('#course_id').val(), starts_at: $('#starts_at').val(), ends_at: $('#ends_at').val() };
                var url = '{{ env("APP_WEB_API_URL") }}/{{ env("APP_WEB_API_VERSION") }}/course-subscriptions' + (id ? '/' + id : '');
                $.ajax({ url: url, type: id ? 'PUT' : 'POST', data: data, dataType: 'JSON', success: function() { location.reload(); } });
            });

            $('#course_subscriptions').on('click', 'button.editor-remove', function(e) {
                e.preventDefault();
                var id = $(this).attr('data-id');
                if (confirm('Are you sure you wish to delete this course subscription?')) {
                    $.ajax({ url: '{{ env("APP_WEB_API_URL") }}/{{ env("APP_WEB_API_VERSION") }}/course-subscriptions/' + id, type: 'DELETE', success: function() { location.reload(); } });
                }
            });
            ...
            $('#course_subscriptions').DataTable({
                processing: true,
                serverSide: true,
                ajax: { url: '{{ env("APP_WEB_API_URL") }}/{{ env("APP_WEB_API_VERSION") }}/course-subscriptions/datatables' },
                columns: [
                    { data: 'id', name: 'id' },
                    { data: 'user', orderable: false, searchable: false, render: function(d) { return d.name; } },
                    { data: 'course', orderable: false, searchable: false, render: function(d) { return d.title + ' ($' + parseFloat(d.price).toFixed(2) + ')'; } },
                    ...
                    },
                   {
                        ...
                        render: function(data, type, row) {
                            ...
                            return '<div class="admin-action-group">'
                                ...
                                + '<button class="admin-action-btn admin-action-btn--edit editor-edit" data-toggle="modal" data-target="#courseSubscriptionModal" data-title="Edit Course Subscription" data-id="' + data + '">' + icEdit + '</button>'
                                + cancelBtn
                                + '</div>';
                        }
                    }
            ...
            // Cancel course subscription (admin)
           $('#course_subscriptions').on('click', 'button.editor-cancel', function(e) {
                ...
                $.ajax({
                    ...
                    url: '/admin/course-subscriptions/' + id + '/cancel',
