...
@php
    $useAppShell = true;
    $overallIndex = null; $overallTotal = null; // computed below if unlocked
@endphp
            ...
            @if (!$unlocked)
                <div class="course-panel">
                    <div class="course-locked-state">
                        ...
                        <h2>This lesson is locked / Esta lección está bloqueada</h2>
                        <p>Complete the previous lesson to unlock this one.<br>Completa la lección anterior para desbloquear esta.</p>
                        <a href="{{ route('course.module', $moduleNumber) }}" class="course-btn course-btn--outline">
                            Back to Module {{ $moduleNumber }} / Volver al módulo {{ $moduleNumber }}
                        </a>
                    </div>
                </div>
            @else
                <div class="course-lesson-progressbar">
                    <div class="course-lesson-progressbar__label">
                        ...
                        @if ($isCompleted)
                            <span class="course-completed-badge"><i class="fa fa-check" aria-hidden="true"></i> Completed / Completado</span>
                        @endif
                    </div>
                ...
                <button type="button" class="course-nav-toggle" id="courseNavToggle" aria-expanded="false">
                    <i class="fa fa-bars" aria-hidden="true"></i> Lesson list / Lista de lecciones
                </button>
                    ...
                    <div class="course-lesson-content">
                        ...
                        @if (in_array($lesson['type'], ['image', 'mixed'], true) && !empty($lesson['image_source']))
                            <div class="course-lesson-media">
                                <img src="{{ asset(ltrim(str_replace('public/', '', $lesson['image_source']), '/')) }}" alt="{{ \Illuminate\Support\Str::limit(strip_tags($lesson['heading'] ?? 'Lesson diagram'), 80) }}">
                            </div>
                        @endif
                        ...
                        @if ($lesson['type'] === 'video' && !empty($lesson['video_files']))
                            <div class="course-lesson-media">
                                    ...
                                    @foreach ($lesson['video_files'] as $vi => $videoFile)
                                        <div class="course-video-block">
                                            <video controls preload="metadata">
                                                <source src="{{ asset('figma/' . $videoFile) }}" type="video/mp4">
                                            </video>
                                ...
                                <p style="font-size:.76rem;color:var(--course-ink-soft, #64748b);margin-top:8px;">
                                    Caption language selection shown for design review only &mdash; live multi-language captions are generated by Bunny Stream once video hosting is connected.<br>
                                    Selección de idioma de subtítulos solo para revisión de diseño.
                                </p>
                            </div>
                        @endif

                        <div class="course-help-box">
                            <p><strong>Didn't understand this lesson? / ¿No entendiste esta lección?</strong><br>Email us and we'll help you through it.</p>
                            <a class="course-btn course-btn--outline course-btn--small"
                               href="mailto:anewavenuebio@gmail.com?subject={{ rawurlencode('Question about Module ' . $moduleNumber . ' — Lesson ' . $lessonIndex . ': ' . trim(str_replace("\n", ' ', $lesson['heading'] ?? ''))) }}&body={{ rawurlencode("Hi Anew Avenue team,\n\nI have a question about Module {$moduleNumber}, Lesson {$lessonIndex}.\n\nMy question:\n") }}">
                                <i class="fa fa-envelope-o" aria-hidden="true"></i> Email about this lesson / Enviar correo
                            </a>
                        </div>
                            ...
                            <div class="course-lesson-actions__nav">
                                @if ($prev)
                                    <a href="{{ route('course.lesson', [$moduleNumber, $prev]) }}" class="course-btn course-btn--ghost">
                                        <i class="fa fa-arrow-left" aria-hidden="true"></i> Previous / Anterior
                                    </a>
                                @elseif ($prevModule)
                            ...
                            <form action="{{ route('course.lesson.complete', [$moduleNumber, $lessonIndex]) }}" method="POST">
                                ...
                                <button type="submit" class="course-btn course-btn--primary">
                                    ...
                                    @elseif (!$next && !$nextModule)
                                        Finish Course / Finalizar curso <i class="fa fa-certificate" aria-hidden="true"></i>
                                    @else
                                        Mark Complete &amp; Continue / Marcar completo y continuar <i class="fa fa-arrow-right" aria-hidden="true"></i>
                                    @endif
                                </button>
                            </form>
                        </div>
